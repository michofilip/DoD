    ...
    def processKeyEvent(gameStage: GameStage, keyEvent: KeyEvent): Seq[Event] = {
        gameStage.gameObjects.findById("player") match {
            case Some(player) => keyEvent.code match {
                ...
                case KeyCode.Numpad2 | KeyCode.Down => Seq(PositionEvent.StepAndFace(Expr(player.id), Expr(Direction.South)))
                case KeyCode.Numpad4 | KeyCode.Left => Seq(PositionEvent.StepAndFace(Expr(player.id), Expr(Direction.West)))
                case _ => Seq.empty
            }
            case None => Seq.empty
        }
