...
object BooleanExpr {
    ...
    final case class Constant(value: Boolean) extends BooleanExpr :
        override def get(using GameObjectRepository): Option[Boolean] = Some(value)

    final case class Not(expr: BooleanExpr) extends BooleanExpr :
        override def get(using GameObjectRepository): Option[Boolean] = expr ~> (x => Some(!x))

    final case class And(expr1: BooleanExpr, expr2: BooleanExpr) extends BooleanExpr :
        override def get(using GameObjectRepository): Option[Boolean] = (expr1, expr2) ~> ((x, y) => Some(x && y))
    ...
    final case class Less[T: Ordering](expr1: Expr[T], expr2: Expr[T]) extends BooleanExpr :
        override def get(using GameObjectRepository): Option[Boolean] = (expr1, expr2) ~> ((x, y) => Some(summon[Ordering[T]].compare(x, y) < 0))
