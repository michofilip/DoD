...
object StringExpr {
    ...
    final case class Constant(value: String) extends StringExpr :
        override def get(using ExprContext): Option[String] = Some(value)

    final case class Concatenate(expr1: StringExpr, expr2: StringExpr) extends StringExpr :
        override def get(using ExprContext): Option[String] = (expr1, expr2) ~> ((x, y) => Some(x + y))

    final case class IntegerToString(expr: IntegerExpr) extends StringExpr :
        override def get(using ExprContext): Option[String] = expr ~> (x => Some(x.toString))
